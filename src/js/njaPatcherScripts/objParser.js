...
export function parseOBJ(obj, objID){
  ...
  return [
    [objVertVector], [objNormalVector], [objTextureVector], [objFaceInfo[0]], [objFaceInfo[1]], [objFaceInfo[0][0],objFaceInfo[0][1],objFaceInfo[0][2]], [normals, texture]
    , objPositionVector
  ]
}
...
export function cyclePushXY(x, y, string){
  function resizeUVCoords(uvCoord){
    let outOfBoundry = false;
    let signUV = false;
    if(uvCoord > 256){
      outOfBoundry = true
    }
    ...
    return parseInt(uvCoord)
  }
  switch(objDimension){
    case 0:
      string = parseInt(string*256)
      //string = resizeUVCoords(string)
      x.push(string)
      objDimension += 1
    break;
    case 1:
